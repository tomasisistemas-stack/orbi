...
type
  TFr_tabela_unidade = class(TForm)
    Panel1: tsPanel;
    Btnov: TsBitBtn;
    btalt: TsBitBtn;
    btgra: TsBitBtn;
  ...
  public
    ...
    procedure ativa_unidade(id: string);
    procedure mostra_campos(prefixo: string);

  end;

var
  Fr_tabela_unidade: TFr_tabela_unidade;
...
procedure TFr_tabela_unidade.ativa_unidade(id: string);
begin
  with q_unidade do
  begin
    close;
    SQL.Clear;
    SQL.add('select * from unidade where unidade = ' + QuotedStr(id));
    open;
  end;
end;

procedure TFr_tabela_unidade.limpa_campos;
var
  i: integer;
begin
...
procedure TFr_tabela_unidade.mostra_campos(prefixo: string);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      TsEdit(FindComponent(campo)).Text := q_unidade.fieldbyname(campo1).AsString;
    end
    else if Components[i] is TsCurrencyEdit then
    begin
      ...
      TsCurrencyEdit(FindComponent(campo)).value := q_unidade.fieldbyname(campo1).value;
    end;
  end;
end;

procedure TFr_tabela_unidade.readonly_false;
var
  i: integer;
begin
  for i := 0 to ComponentCount - 1 do
...
procedure TFr_tabela_unidade.readonly_true;
var
  i: integer;
begin
  for i := 0 to ComponentCount - 1 do
...
procedure TFr_tabela_unidade.btaltClick(Sender: TObject);
begin
...
procedure TFr_tabela_unidade.BtnovClick(Sender: TObject);
begin
  Limpa_campos;
  modo_insert := true;
  readonly_false;
...
procedure TFr_tabela_unidade.btgraClick(Sender: TObject);
begin
  try
    ...
    if modo_insert then
    begin
      id := dao.insert('Pr', 'unidade', '', Fr_tabela_unidade, '');
      //Prunidade.Text := id;
//      ativa_unidade(id);
      btNov.setFocus;
    end
    else
      dao.update('unidade', 'unidade', unidade, 'Pr', Fr_tabela_unidade);
    dao.cn.commit;
    readonly_true;
  except
    dao.cn.Rollback;
...
procedure TFr_tabela_unidade.btcanClick(Sender: TObject);
var
...
procedure TFr_tabela_unidade.btexcClick(Sender: TObject);
begin
  if Prunidade.Text = '' then
  begin
  ...
  if MessageDlg('Tem Certeza que Deseja Excluir esta unidade?', mtConfirmation, [mbyes, mbno], 0) = mryes then
  begin
    try
      if not (dao.cn.InTransaction) then
        dao.cn.StartTransaction;
      dao.delete('Prunidade', q_unidade.fieldbyname('unidade').AsString, 'unidade');
      dao.cn.Commit;
      dao.msg('Unidade excluida com sucesso!');
    except
      dao.cn.rollback;
    end;
  end;
...
procedure TFr_tabela_unidade.BtproClick(Sender: TObject);
var
...
procedure TFr_tabela_unidade.FormCloseQuery(Sender: TObject;
...
procedure TFr_tabela_unidade.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
  begin
    Key := #0;
...
procedure TFr_tabela_unidade.PrunidadeKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
  begin
...
procedure TFr_tabela_unidade.FormShow(Sender: TObject);
begin
