...
uses
  ...
  Menus, StdCtrls, ExtCtrls, Mask, Buttons, sPanel, sBitBtn,
  RLFilters, RLXLSFilter, RLReport, sLabel, sCheckBox, sGroupBox, sEdit,
  RLConsts, RLTypes, RLUtils, RLFeedBack, RLPrinters, RLMETAfile;
type
  TFormSendMailOptions = class(TForm)
    Label1: tsLabel;
    Label2: tsLabel;
    Label3: tsLabel;
    GroupBoxFormato: TsGroupBox;
    BitBtnEnviar: TsBitBtn;
    BitBtnCancelar: TsBitBtn;
    EditDestinatarios: TsEdit;
    EditAssunto: TsEdit;
    RadioButtonEXE: TRadioButton;
    RadioButtonPDF: TRadioButton;
    RadioButtonRTF: TRadioButton;
    RadioButtonHTM: TRadioButton;
    CheckBoxAbrir: tscheckbox;
    PanelOpcoes: tsPanel;
    PanelSubmit: tsPanel;
...
procedure TFormSendMailOptions.FormActivate(Sender: TObject);
begin
  {if DMVEN.QUVE1.FieldByName('emacli').AsString<>'' then
  EditDestinatarios.Text:=DMVEN.QUVE1.FieldByName('emacli').AsString; }
