object fr_plano_contas_consulta: Tfr_plano_contas_consulta
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMaximize]
  Caption = 'Plano de Contas'
  ClientHeight = 589
  ClientWidth = 1108
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TsPanel
    Left = 0
    Top = 0
    Width = 1108
    Height = 49
    Align = alTop
    BevelInner = bvLowered
    TabOrder = 0
    SkinData.SkinSection = 'PANEL'
    object btImprimir: TsSpeedButton
      Left = 914
      Top = 13
      Width = 86
      Height = 25
      Caption = '&Imprimir'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      NumGlyphs = 2
      ParentFont = False
      OnClick = btImprimirClick
      SkinData.SkinSection = 'SPEEDBUTTON'
    end
    object GroupBox1: TsGroupBox
      Left = 5
      Top = 2
      Width = 416
      Height = 43
      Caption = 'Informe a Descri'#231#227'o '#224' ser Pesquisada:'
      TabOrder = 0
      SkinData.SkinSection = 'GROUPBOX'
      object edDesc: TsEdit
        Left = 8
        Top = 16
        Width = 401
        Height = 21
        CharCase = ecUpperCase
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        SkinData.SkinSection = 'EDIT'
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
      end
    end
    object BtLocalizar: TsBitBtn
      Left = 787
      Top = 13
      Width = 75
      Height = 25
      Caption = '&Localizar'
      Glyph.Data = {
        
        4747474737060E4C4C4C4C4C161010101010101010064C4C4C4C}
      TabOrder = 1
      OnClick = BtLocalizarClick
      SkinData.SkinSection = 'BUTTON'
    end
    object GroupBox4: TsGroupBox
      Left = 439
      Top = 2
      Width = 264
      Height = 43
      Caption = 'Data Movimento'
      TabOrder = 2
      SkinData.SkinSection = 'GROUPBOX'
      object Label1: TsLabel
        Left = 8
        Top = 18
        Width = 17
        Height = 13
        Caption = 'De:'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object Label2: TsLabel
        Left = 136
        Top = 18
        Width = 19
        Height = 13
        Caption = 'At'#233':'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object ftMovDe: TsDateEdit
        Left = 27
        Top = 15
        Width = 99
        Height = 21
        AutoSize = False
        Color = clWhite
        EditMask = '!99/99/9999;1; '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 10
        ParentFont = False
        TabOrder = 0
        Text = '  /  /    '
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
        GlyphMode.Blend = 0
        GlyphMode.Grayed = False
        DialogTitle = 'Selecione uma Data'
      end
      object ftMovAte: TsDateEdit
        Left = 159
        Top = 15
        Width = 99
        Height = 21
        AutoSize = False
        Color = clWhite
        EditMask = '!99/99/9999;1; '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 10
        ParentFont = False
        TabOrder = 1
        Text = '  /  /    '
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
        GlyphMode.Blend = 0
        GlyphMode.Grayed = False
        DialogTitle = 'Selecione uma Data'
      end
    end
  end
  object Panel7: TsPanel
    Left = 0
    Top = 49
    Width = 595
    Height = 459
    Align = alClient
    BevelOuter = bvLowered
    TabOrder = 1
    SkinData.SkinSection = 'PANEL'
    object GroupBox2: TsGroupBox
      Left = 1
      Top = 1
      Width = 593
      Height = 137
      Align = alClient
      Caption = 'Ativo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      SkinData.SkinSection = 'GROUPBOX'
      object Panel2: TsPanel
        Left = 2
        Top = 106
        Width = 589
        Height = 29
        Align = alBottom
        BevelOuter = bvLowered
        TabOrder = 0
        SkinData.SkinSection = 'PANEL'
        object stAtivo: TsLabel
          Left = 410
          Top = 1
          Width = 178
          Height = 25
          Align = alRight
          Alignment = taRightJustify
          Caption = 'Total Ativo:R$ 0,00'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Sylfaen'
          Font.Style = [fsBold]
        end
      end
      object TvAtivo: TTreeView
        Left = 2
        Top = 22
        Width = 589
        Height = 84
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Indent = 19
        ParentFont = False
        TabOrder = 1
      end
    end
    object GroupBox5: TsGroupBox
      Left = 1
      Top = 138
      Width = 593
      Height = 320
      Align = alBottom
      Caption = 'Receitas '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      SkinData.SkinSection = 'GROUPBOX'
      object tvReceitas: TTreeView
        Left = 2
        Top = 22
        Width = 589
        Height = 269
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Indent = 19
        ParentFont = False
        TabOrder = 0
      end
      object Panel6: TsPanel
        Left = 2
        Top = 291
        Width = 589
        Height = 27
        Align = alBottom
        BevelOuter = bvLowered
        TabOrder = 1
        SkinData.SkinSection = 'PANEL'
        object stReceita: TsLabel
          Left = 381
          Top = 1
          Width = 207
          Height = 25
          Align = alRight
          Alignment = taRightJustify
          Caption = 'Total Receitas: R$ 0,00'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Sylfaen'
          Font.Style = [fsBold]
          ExplicitLeft = 289
        end
      end
    end
  end
  object Panel8: TsPanel
    Left = 595
    Top = 49
    Width = 513
    Height = 459
    Align = alRight
    BevelInner = bvLowered
    BevelOuter = bvLowered
    TabOrder = 2
    SkinData.SkinSection = 'PANEL'
    object GroupBox6: TsGroupBox
      Left = 2
      Top = 137
      Width = 509
      Height = 320
      Align = alBottom
      Caption = 'Despesas e Custos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      SkinData.SkinSection = 'GROUPBOX'
      object Panel5: TsPanel
        Left = 2
        Top = 291
        Width = 505
        Height = 27
        Align = alBottom
        BevelOuter = bvLowered
        TabOrder = 0
        SkinData.SkinSection = 'PANEL'
        object stDespesa: TsLabel
          Left = 197
          Top = 1
          Width = 307
          Height = 25
          Align = alRight
          Alignment = taRightJustify
          Caption = 'Total Despesas ou Custos: R$ 0,00'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Sylfaen'
          Font.Style = [fsBold]
        end
      end
      object tvDespesas: TTreeView
        Left = 2
        Top = 22
        Width = 505
        Height = 269
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Indent = 19
        ParentFont = False
        TabOrder = 1
      end
    end
    object GroupBox3: TsGroupBox
      Left = 2
      Top = 2
      Width = 509
      Height = 135
      Align = alClient
      Caption = 'Passivo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      SkinData.SkinSection = 'GROUPBOX'
      object TvPassivo: TTreeView
        Left = 2
        Top = 22
        Width = 505
        Height = 84
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Indent = 19
        ParentFont = False
        TabOrder = 0
      end
      object Panel3: TsPanel
        Left = 2
        Top = 106
        Width = 505
        Height = 27
        Align = alBottom
        BevelOuter = bvLowered
        TabOrder = 1
        SkinData.SkinSection = 'PANEL'
        object stPassivo: TsLabel
          Left = 310
          Top = 1
          Width = 194
          Height = 25
          Align = alRight
          Alignment = taRightJustify
          Caption = 'Total Passivo:R$ 0,00'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Sylfaen'
          Font.Style = [fsBold]
        end
      end
    end
  end
  object Panel9: TsPanel
    Left = 0
    Top = 535
    Width = 1108
    Height = 27
    Align = alBottom
    BevelOuter = bvLowered
    TabOrder = 3
    SkinData.SkinSection = 'PANEL'
    object stTotalGeral: TsLabel
      Left = 912
      Top = 1
      Width = 195
      Height = 27
      Align = alRight
      Alignment = taRightJustify
      Caption = 'Total Ativo:R$ 0,00'
      ParentFont = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -21
      Font.Name = 'Sylfaen'
      Font.Style = [fsBold]
    end
  end
  object tsPanel: TsPanel
    Left = 0
    Top = 562
    Width = 1108
    Height = 27
    Align = alBottom
    BevelOuter = bvLowered
    TabOrder = 4
    SkinData.SkinSection = 'PANEL'
    object stTotalGeralLiq: TsLabel
      Left = 912
      Top = 1
      Width = 195
      Height = 27
      Align = alRight
      Alignment = taRightJustify
      Caption = 'Total Ativo:R$ 0,00'
      ParentFont = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -21
      Font.Name = 'Sylfaen'
      Font.Style = [fsBold]
    end
  end
  object Panel4: TsPanel
    Left = 0
    Top = 508
    Width = 1108
    Height = 27
    Align = alBottom
    BevelOuter = bvLowered
    TabOrder = 5
    SkinData.SkinSection = 'PANEL'
    object stDespesasGeral: TsLabel
      Left = 803
      Top = 1
      Width = 304
      Height = 25
      Align = alRight
      Alignment = taRightJustify
      Caption = 'Total (Passivo+Despesas): R$ 0,00'
      ParentFont = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -19
      Font.Name = 'Sylfaen'
      Font.Style = [fsBold]
      ExplicitLeft = 711
    end
    object stReceitasGeral: TsLabel
      Left = 1
      Top = 1
      Width = 500
      Height = 25
      Align = alLeft
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Total (Ativo+Receitas): R$ 0,00'
      ParentFont = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -19
      Font.Name = 'Sylfaen'
      Font.Style = [fsBold]
    end
  end
  object Q_Ativo: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 144
    Top = 74
  end
  object q_despesas: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 152
    Top = 170
  end
  object q_receitas: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 296
    Top = 210
  end
  object fR_RELATORIO: TfrxReport
    Version = '6.9.3'
    DotMatrixReport = False
    EngineOptions.PrintIfEmpty = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40180.423318680600000000
    ReportOptions.LastChange = 40560.616978333330000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'procedure mmDescricao1OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      '  if (<dst_plano_contas."OPERACAO"> = '#39'(-)'#39') then'
      '  begin              '
      '    mmDescricao1.Font.Color := clMaroon;'
      '    mmDescricao2.Font.Color := clMaroon;        '
      '    mmDescricao3.Font.Color := clMaroon;'
      '    mmDescricao4.Font.Color := clMaroon;'
      '    lbData.Font.Color       := clMaroon;  '
      '    lbCliente.Font.Color    := clMaroon;  '
      '    lbValor.Font.Color      := clMaroon;  '
      '    lbTitulo.Font.Color     := clMaroon;'
      '    mmData.Font.Color       := clMaroon;  '
      '    mmValor.Font.Color      := clMaroon;  '
      '    mmCliente.Font.Color    := clMaroon;  '
      '    mmTitulo.Font.Color     := clMaroon;'
      '    mmTotal1.Font.Color     := clMaroon;  '
      '    mmTotal2.Font.Color     := clMaroon;  '
      '    mmTotal3.Font.Color     := clMaroon;  '
      '    mmTotal4.Font.Color     := clMaroon;'
      '    mmTotalBloco.Font.Color := clMaroon;'
      
        '    mmTotalBloco.memo.text  := '#39'Total Despesas: [SUM(<dst_plano_' +
        'contas."VALOR">,DetailData1,1)]'#39';'
      '    mmHistorico.Font.Color  := clMaroon;'
      
        '    lbHistorico.Font.Color  := clMaroon;                        ' +
        '      '
      '  end'
      '  else begin             '
      '    mmDescricao1.Font.Color := clBlack;'
      '    mmDescricao2.Font.Color := clBlack;        '
      '    mmDescricao3.Font.Color := clBlack;'
      '    mmDescricao4.Font.Color := clBlack;'
      '    lbData.Font.Color       := clBlack;  '
      '    lbCliente.Font.Color    := clBlack;  '
      '    lbValor.Font.Color      := clBlack;  '
      '    lbTitulo.Font.Color     := clBlack;'
      '    mmData.Font.Color       := clBlack;  '
      '    mmValor.Font.Color      := clBlack;  '
      '    mmCliente.Font.Color    := clBlack;  '
      '    mmTitulo.Font.Color     := clBlack;'
      '    mmTotal1.Font.Color     := clBlack;  '
      '    mmTotal2.Font.Color     := clBlack;  '
      '    mmTotal3.Font.Color     := clBlack;  '
      '    mmTotal4.Font.Color     := clBlack;'
      '    mmTotalBloco.Font.Color := clBlack;'
      
        '    mmTotalBloco.memo.text  := '#39'Total Receitas: [SUM(<dst_plano_' +
        'contas."VALOR">,DetailData1,1)]'#39';'
      '    mmHistorico.Font.Color  := clBlack;'
      
        '    lbHistorico.Font.Color  := clBlack;                         ' +
        '     '
      '        '
      '  end;              '
      'end;'
      ''
      'begin'
      ''
      'end.')
    OnGetValue = fR_RELATORIOGetValue
    Left = 456
    Top = 40
    Datasets = <
      item
        DataSet = Dm.dst_plano_contas
        DataSetName = 'dst_plano_contas'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Frame.Typ = []
        Height = 7.559060000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 200.315090000000000000
        Width = 718.110700000000000000
        DataSet = Dm.dst_plano_contas
        DataSetName = 'dst_plano_contas'
        RowCount = 0
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 854.173780000000000000
        Width = 718.110700000000000000
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Left = 642.520100000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[Page#]')
        end
      end
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Frame.Typ = []
        Height = 90.708720000000000000
        Top = 49.133890000000000000
        Width = 718.110700000000000000
        object SysMemo1: TfrxSysMemoView
          AllowVectorExport = True
          Left = 616.842920000000000000
          Top = 1.000000000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[DATE]')
          ParentFont = False
        end
        object SysMemo2: TfrxSysMemoView
          AllowVectorExport = True
          Left = 573.488560000000000000
          Top = 19.897650000000010000
          Width = 139.842610000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[PAGE#] de [TOTALPAGES#]')
          ParentFont = False
        end
        object Memo24: TfrxMemoView
          AllowVectorExport = True
          Left = 7.559060000000000000
          Top = 67.354360000000010000
          Width = 298.582870000000000000
          Height = 18.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Plano de Contas')
          ParentFont = False
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 313.700990000000000000
          Top = 67.354360000000010000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[dtaini]')
          ParentFont = False
        end
        object Memo26: TfrxMemoView
          AllowVectorExport = True
          Left = 400.630180000000000000
          Top = 67.354360000000010000
          Width = 15.118120000000000000
          Height = 18.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'e')
          ParentFont = False
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 423.307360000000000000
          Top = 67.354360000000010000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            '[dtafin]')
          ParentFont = False
        end
        object Picture1: TfrxPictureView
          AllowVectorExport = True
          Left = 7.559060000000000000
          Top = 3.779530000000001000
          Width = 200.315090000000000000
          Height = 56.692950000000000000
          Frame.Typ = []
          Picture.Data = {
            
            96FF007BFA0A27B04773FFD9}
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo8: TfrxMemoView
          AllowVectorExport = True
          Left = 211.653680000000000000
          Top = 3.779530000000001000
          Width = 415.748300000000000000
          Height = 18.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            '[empresa]')
          ParentFont = False
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 211.653680000000000000
          Top = 20.456709999999990000
          Width = 415.748300000000000000
          Height = 37.795300000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'www.plasfan.ind.br'
            'Email: plasfan@terra.com.br'
            'Fone: (47)3346-2052')
          ParentFont = False
        end
      end
      object DetailData1: TfrxDetailData
        FillType = ftBrush
        Frame.Typ = []
        Height = 15.118120000000000000
        Top = 468.661720000000000000
        Width = 718.110700000000000000
        OnAfterCalcHeight = 'DetailData1OnAfterCalcHeight'
        OnAfterPrint = 'DetailData1OnAfterPrint'
        OnBeforePrint = 'DetailData1OnBeforePrint'
        DataSet = Dm.dst_plano_contas
        DataSetName = 'dst_plano_contas'
        KeepChild = True
        RowCount = 0
        object mmData: TfrxMemoView
          AllowVectorExport = True
          Left = 37.795300000000000000
          Top = 0.779530000000022500
          Width = 52.913420000000000000
          Height = 11.338590000000000000
          DataField = 'DATA'
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[dst_plano_contas."DATA"]')
          ParentFont = False
        end
        object mmValor: TfrxMemoView
          AllowVectorExport = True
          Left = 627.842920000000000000
          Top = 0.779530000000022500
          Width = 86.929190000000000000
          Height = 11.338590000000000000
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[dst_plano_contas."VALOR"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object mmCliente: TfrxMemoView
          AllowVectorExport = True
          Left = 94.488250000000000000
          Top = 0.779530000000022500
          Width = 245.669450000000000000
          Height = 11.338590000000000000
          OnBeforePrint = 'Memo5OnBeforePrint'
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            '[dst_plano_contas."CLIENTE_FORNECEDOR"]')
          ParentFont = False
        end
        object mmTitulo: TfrxMemoView
          AllowVectorExport = True
          Left = 555.590910000000000000
          Top = 1.000000000000000000
          Width = 68.031540000000000000
          Height = 11.338590000000000000
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[dst_plano_contas."TITULO"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object mmHistorico: TfrxMemoView
          AllowVectorExport = True
          Left = 341.937230000000000000
          Top = 1.000000000000000000
          Width = 211.653543310000000000
          Height = 11.338590000000000000
          OnBeforePrint = 'Memo5OnBeforePrint'
          DataField = 'HISTORICO'
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            '[dst_plano_contas."HISTORICO"]')
          ParentFont = False
        end
      end
      object GroupHeader2: TfrxGroupHeader
        FillType = ftBrush
        Frame.Typ = []
        Height = 35.236240000000000000
        Top = 411.968770000000000000
        Width = 718.110700000000000000
        Condition = 'dst_plano_contas."DESCRICAO4"'
        KeepChild = True
        KeepTogether = True
        object mmDescricao4: TfrxMemoView
          AllowVectorExport = True
          Left = 37.795300000000000000
          Top = 1.779530000000022000
          Width = 597.165740000000000000
          Height = 15.118120000000000000
          DataSet = Dm.dst_comissao_nova
          DataSetName = 'dst_comissao_nova'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[dst_plano_contas."DESCRICAO4"]')
          ParentFont = False
        end
        object lbCliente: TfrxMemoView
          AllowVectorExport = True
          Left = 93.708720000000000000
          Top = 22.118119999999920000
          Width = 245.669450000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftBottom]
          Memo.UTF8W = (
            'Cliente/Fornecedor')
          ParentFont = False
        end
        object lbData: TfrxMemoView
          AllowVectorExport = True
          Left = 37.692950000000000000
          Top = 21.897649999999880000
          Width = 52.913420000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Data')
          ParentFont = False
        end
        object lbValor: TfrxMemoView
          AllowVectorExport = True
          Left = 627.961040000000000000
          Top = 21.897649999999880000
          Width = 86.929190000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Valor')
          ParentFont = False
        end
        object lbTitulo: TfrxMemoView
          AllowVectorExport = True
          Left = 555.590910000000000000
          Top = 21.677180000000020000
          Width = 68.031540000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'T'#237'tulo')
          ParentFont = False
        end
        object lbHistorico: TfrxMemoView
          AllowVectorExport = True
          Left = 342.157700000000000000
          Top = 21.897649999999880000
          Width = 211.653680000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftBottom]
          Memo.UTF8W = (
            'Hist'#243'rico')
          ParentFont = False
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        Frame.Typ = []
        Height = 18.897650000000000000
        Top = 506.457020000000000000
        Width = 718.110700000000000000
        object mmTotal4: TfrxMemoView
          AllowVectorExport = True
          Left = 529.134200000000000000
          Width = 185.196970000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop]
          HAlign = haRight
          Memo.UTF8W = (
            
              ' [dst_plano_contas."DESCRICAO4"] - Total : [SUM(<dst_plano_conta' +
              's."VALOR">,DetailData1)]')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        Frame.Typ = []
        Height = 18.897650000000000000
        Top = 370.393940000000000000
        Width = 718.110700000000000000
        Condition = 'dst_plano_contas."DESCRICAO3"'
        object mmDescricao3: TfrxMemoView
          AllowVectorExport = True
          Left = 26.456710000000000000
          Top = 0.779530000000022500
          Width = 631.181510000000000000
          Height = 15.118120000000000000
          DataSet = Dm.dst_comissao_nova
          DataSetName = 'dst_comissao_nova'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[dst_plano_contas."DESCRICAO3"]')
          ParentFont = False
        end
      end
      object GroupHeader3: TfrxGroupHeader
        FillType = ftBrush
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 325.039580000000000000
        Width = 718.110700000000000000
        Condition = 'dst_plano_contas."DESCRICAO2"'
        object mmDescricao2: TfrxMemoView
          AllowVectorExport = True
          Left = 15.118120000000000000
          Width = 665.197280000000000000
          Height = 18.897650000000000000
          DataSet = Dm.dst_comissao_nova
          DataSetName = 'dst_comissao_nova'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[dst_plano_contas."DESCRICAO2"]')
          ParentFont = False
        end
      end
      object GroupHeader4: TfrxGroupHeader
        FillType = ftBrush
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 279.685220000000000000
        Width = 718.110700000000000000
        Condition = 'dst_plano_contas."DESCRICAO1"'
        object mmDescricao1: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Width = 665.197280000000000000
          Height = 18.897650000000000000
          OnBeforePrint = 'mmDescricao1OnBeforePrint'
          DataSet = Dm.dst_comissao_nova
          DataSetName = 'dst_comissao_nova'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[dst_plano_contas."DESCRICAO1"]')
          ParentFont = False
        end
      end
      object GroupFooter2: TfrxGroupFooter
        FillType = ftBrush
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 593.386210000000000000
        Width = 718.110700000000000000
        object mmTotal3: TfrxMemoView
          AllowVectorExport = True
          Left = 529.134200000000000000
          Width = 185.196970000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop]
          HAlign = haRight
          Memo.UTF8W = (
            
              ' [dst_plano_contas."DESCRICAO2"] - Total : [SUM(<dst_plano_conta' +
              's."VALOR">,DetailData1)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
      end
      object GroupFooter3: TfrxGroupFooter
        FillType = ftBrush
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 548.031849999999900000
        Width = 718.110700000000000000
        object mmTotal2: TfrxMemoView
          AllowVectorExport = True
          Left = 529.134200000000000000
          Width = 185.196970000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop]
          HAlign = haRight
          Memo.UTF8W = (
            
              '[dst_plano_contas."DESCRICAO3"] - Total : [SUM(<dst_plano_contas' +
              '."VALOR">,DetailData1)]')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object GroupFooter4: TfrxGroupFooter
        FillType = ftBrush
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 638.740570000000000000
        Width = 718.110700000000000000
        object mmTotal1: TfrxMemoView
          AllowVectorExport = True
          Left = 529.134200000000000000
          Width = 185.196970000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop]
          HAlign = haRight
          Memo.UTF8W = (
            
              ' [dst_plano_contas."DESCRICAO1"] - Total : [SUM(<dst_plano_conta' +
              's."VALOR">,DetailData1)]')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Frame.Typ = []
        Height = 60.472480000000000000
        Top = 771.024120000000000000
        Width = 718.110700000000000000
        KeepChild = True
        object Memo2: TfrxMemoView
          AllowVectorExport = True
          Left = 529.134200000000000000
          Top = 30.236239999999960000
          Width = 185.196970000000000000
          Height = 22.677180000000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -17
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          Frame.Typ = [ftTop]
          HAlign = haRight
          Memo.UTF8W = (
            
              'Saldo Total: [<SUM(<dst_plano_contas."VALOR_SALDO">,DetailData1)' +
              '>]')
          ParentFont = False
          WordWrap = False
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 529.134200000000000000
          Width = 185.196970000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -17
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          Frame.Typ = [ftTop]
          HAlign = haRight
          Memo.UTF8W = (
            
              'Saldo Movimenta'#231#227'o: [SUM(<dst_plano_contas."VALOR_SALDO">,Detail' +
              'Data1)]')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object GroupHeader5: TfrxGroupHeader
        FillType = ftBrush
        Frame.Typ = []
        Height = 11.338590000000000000
        Top = 245.669450000000000000
        Width = 718.110700000000000000
        Condition = 'dst_plano_contas."OPERACAO"'
        object mmOperacao: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Width = 665.197280000000000000
          Height = 3.779530000000000000
          Visible = False
          OnBeforePrint = 'mmDescricao1OnBeforePrint'
          DataField = 'OPERACAO'
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[dst_plano_contas."OPERACAO"]')
          ParentFont = False
        end
      end
      object GroupFooter5: TfrxGroupFooter
        FillType = ftBrush
        Frame.Typ = []
        Height = 26.456710000000000000
        Top = 684.094930000000000000
        Width = 718.110700000000000000
        object mmTotalBloco: TfrxMemoView
          AllowVectorExport = True
          Left = 529.134200000000000000
          Top = 1.000000000000000000
          Width = 185.196970000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = Dm.dst_plano_contas
          DataSetName = 'dst_plano_contas'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsItalic]
          Frame.Typ = [ftTop]
          HAlign = haRight
          Memo.UTF8W = (
            ' [SUM(<dst_plano_contas."VALOR">,DetailData1,1)]')
          ParentFont = False
          VAlign = vaBottom
        end
      end
    end
  end
  object Q_Plano_Conta1: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 184
    Top = 234
  end
  object Q_Plano_Conta2: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 232
    Top = 242
  end
  object Q_Plano_Conta3: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 200
    Top = 282
  end
  object Q_Plano_Conta4: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 240
    Top = 282
  end
  object Q_Passivo: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      '')
    Left = 288
    Top = 114
  end
end
