...
function CallStackTextualRepresentation(const S: TCallStack;
  const LineHeader: string): string;
// Will contain CR/LFs
...
const
  ...
  CheckWipedBlocksOnTermination = True and WipeOutMemoryOnFreeMem and
    not(DanglingInterfacesVerified);
...
type
  ...
  TMemoryBlocHeader = record
    ...
    // this is an integer because the parameter of GetMem is an integer
 ...
 { TFieldInfo = class
    ...
    constructor Create(const TheOwnerClass: TClass;
      const TheFieldIndex: Integer);
  end;   }
...
type
  ...
  public
    ...
    constructor Create(const AName: string; const AStartAddress: Cardinal;
...
begin
  ...
  if _file <> '' then
    Result := Format('($%p) %s:%s:%d (%s)', [Addr, _module, _proc,
      _line, _file])
  else
...
begin
  Result := (Old.TotalAddrSpace <> New.TotalAddrSpace) or
    ...
    (Old.TotalFree <> New.TotalFree) or (Old.FreeSmall <> New.FreeSmall) or
    (Old.FreeBig <> New.FreeBig) or (Old.Unused <> New.Unused) or
    (Old.Overhead <> New.Overhead) or (Old.HeapErrorCode <> New.HeapErrorCode)
    or (New.TotalUncommitted + New.TotalCommitted <> New.TotalAddrSpace) or
    (New.Unused + New.FreeBig + New.FreeSmall <> New.TotalFree)
end;

class function TIntegerBinaryTree.StoredValue(const Address: Cardinal)
  : Cardinal;
begin
...
{
constructor TFieldInfo.Create(const TheOwnerClass: TClass;
  const TheFieldIndex: Integer);
begin
  inherited Create;
...
const
  TObjectVirtualMethodNames: array [1 .. 8] of string = ('SafeCallException',
...
begin
  ...
  try
    ...
    FeedBackStr := FeedBackStr + ' on a FREED instance of ' + t.VMT.ClassName +
      ' (destroyed at ' + TextualDebugInfoForAddress
      (Cardinal(t.DestructionAdress)) + ' - had been created at ' +
  ...
  if ShowCallStack then
    for I := 1 to StoredCallStackDepth do
      if Integer(t.CallerAddress[I]) > 0 then
        try
          raise EStackUnwinding.Create('Unwinding level ' + chr(ord('0') + I))
            at t.CallerAddress[I]
        except
...
begin
  try
    OutputFileHeader := OutputFileHeader +
      #13#10'Exception: Calling an interface method on an freed Pascal instance @ '
      + TextualDebugInfoForAddress(Cardinal(InterfaceErrorCaller)) + #13#10;
    raise EInterfaceFreedInstance.Create
      ('Calling an interface method on an freed Pascal instance')
      at InterfaceErrorCaller
  except
...
var
  StackStart: Pointer;
  StackMax: Pointer;
  // the stack can never go beyond - http://msdn.microsoft.com/library/periodic/period96/S2CE.htm
...
begin
  ...
  count := 0;
  SkipCount := 0;
  while (longint(CurrentFrame) >= longint(StackStart)) and
    (longint(CurrentFrame) < longint(StackMax)) and
    (count <= StoredCallStackDepth) do
  begin
...
{$IFDEF DELPHI6_OR_LATER}
        ...
        begin
          ...
          PMemoryBlocHeader(Result).CallerAddress[0] :=
            ltgmCallerOfTObjectCreate;
        end
        else
{$ENDIF}
    ...
    begin // Neither an object nor a string, this is a MUser
      Result := OldMemoryManager.GetMem
        (Size + (SizeOf(TMemoryBlocHeader) + SizeOf(TMemoryBlockFooter)));
      if Result = nil then
        raise OutOfMemory;
      ...
      PMemoryBlockFooter(PChar(Result) + SizeOf(TMemoryBlocHeader) + Size)^ :=
        EndOfBlock;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          UnMemChk;
          ReallocMem(MemoryUsageStats, MemoryUsageStatsAllocatedPos *
            SizeOf(Integer));
          MemChk;
        end;
...
begin
  ...
  Result := nil;
  // Note: I don't understand right now why I get a warning if I suppress this line
  try
...
begin
  if (Block.KindOfBlock = MClass) then
    Result := False
    // We don't put a footer for objects - This could be done if interesting
  else
    Result := PMemoryBlockFooter(PChar(Block) + SizeOf(TMemoryBlocHeader) +
      Block.AllocatedSize)^ <> EndOfBlock;
end;
...
procedure AddFieldInfo(const FieldAddress: Pointer; const OwnerClass: TClass;
  const FieldPos: Integer);
begin
  UnMemChk;
  ...
  begin
    ...
    ReallocMem(NotDestroyedFields, NotDestroyedFieldsAllocatedSpace *
      SizeOf(Integer));
    ReallocMem(NotDestroyedFieldsInfos, NotDestroyedFieldsAllocatedSpace *
      SizeOf(Integer));
  end;
...
begin
  ...
  begin
      ...
      if (Block.KindOfBlock = MClass) and
        ...
        for I := 1 to (Block.AllocatedSize div 4) - 1 do
          if (PInteger(PChar(P) + I * 4)^ > SizeOf(TMemoryBlocHeader)) and
            CurrentlyAllocatedBlocksTree.Has(PInteger(PChar(P) + I * 4)^ -
            SizeOf(TMemoryBlocHeader)) then
            AddFieldInfo(Pointer(PInteger(PChar(P) + I * 4)^ -
              SizeOf(TMemoryBlocHeader)), Tobject(P).ClassType, I);

      UnMemChk;
      if not MemoryBlockFreed(Block) then
      begin
        Assert(CurrentlyAllocatedBlocksTree.Has(Integer(Block)),
          'freemem: block not among allocated ones');
    ...
    begin
      try
        OutputFileHeader := OutputFileHeader +
          #13#10'Exception: second release of block attempt, allocated at ' +
          TextualDebugInfoForAddress(Cardinal(Block.CallerAddress[0])) +
          ' - Already freed at ' + TextualDebugInfoForAddress
          (Cardinal(Block.DestructionAdress)) + #13#10;
        raise EMemoryLeak.Create
          ('second release of block attempt, already freed')
          at Block.DestructionAdress;
      except
        on EMemoryLeak do;
      end;
      ...
      if ShowCallStack then
        for I := 1 to StoredCallStackDepth do
          if Integer(Block.CallerAddress[I]) > 0 then
            try
              raise EStackUnwinding.Create('Unwinding level ' +
                chr(ord('0') + I))at Block.CallerAddress[I]
            except
              on EStackUnwinding do;
            end;
      ...
      begin
        try
          OutputFileHeader := OutputFileHeader +
            ...
            + TextualDebugInfoForAddress(Cardinal(BlockAllocationAddress(P))
            ) + #13#10;
          raise EMemoryLeak.Create
            ('memory damaged beyond block allocated space, allocated at ' +
            TextualDebugInfoForAddress(Cardinal(BlockAllocationAddress(P))))
            at CallerOfCaller;
        except
          on EMemoryLeak do;
...
{$ENDIF}
        if WipeOutMemoryOnFreeMem then
          ...
          begin
            ...
            FillChar((PChar(P) + 4)^, Block.AllocatedSize - 4,
              CharToUseToWipeOut);
...
begin
  ...
  begin
    ...
    if Size > PMemoryBlocHeader(PChar(P) - SizeOf(TMemoryBlocHeader)).AllocatedSize
    then
      move(P^, Result^, PMemoryBlocHeader(PChar(P) - SizeOf(TMemoryBlocHeader))
        .AllocatedSize)
    else
      move(P^, Result^, Size);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (BlocksToShow[Block.KindOfBlock]) and
            ((Block.KindOfBlock <> MClass) or
            (Tobject(PChar(Block) + SizeOf(TMemoryBlocHeader))
            is InstancesConformingToForReporting)) then
            try
              raise EMemoryLeak.Create(S + ' allocated at ' +
                TextualDebugInfoForAddress(Cardinal(Block.CallerAddress[0])))
                at Block.CallerAddress[0];
            except
              on EMemoryLeak do;
            end;
          ...
          if ShowCallStack then
            for I := 1 to StoredCallStackDepth do
              if Integer(Block.CallerAddress[I]) > 0 then
                try
                  raise EStackUnwinding.Create(S + ' unwinding level ' +
                    chr(ord('0') + I))at Block.CallerAddress[I]
                except
                  on EStackUnwinding do;
                end;
          ...
          { this is not a leak }
          if CheckWipedBlocksOnTermination and (Block.AllocatedSize > 5) and
            (Block.AllocatedSize <= DoNotCheckWipedBlocksBiggerThan) and
            (not IsMemFilledWithChar(PChar(Block) + SizeOf(TMemoryBlocHeader) +
            4, Block.AllocatedSize - 5, CharToUseToWipeOut)) then
          begin
            try
              raise EFreedBlockDamaged.Create
                ('Destroyed block damaged - Block allocated at ' +
                TextualDebugInfoForAddress(Cardinal(Block.CallerAddress[0])) +
                ' - destroyed at ' + TextualDebugInfoForAddress
                (Cardinal(Block.DestructionAdress)))at Block.CallerAddress[0]
            except
...
begin
  (* UnitsInfo := PInitContext(PAnsiChar(@AllocMemSize) + 8728).InitTable;
...
begin
  ...
  begin
    ...
    if TUnitDebugInfos(Units[Pivot]).Addresses[0].Address > Address then
      ...
      [Length(TUnitDebugInfos(Units[Pivot]).Addresses) - 1].Address < Address
    then
      Start := Pivot + 1
    else
    begin
...
begin
  ...
  Writeln(SevereExceptionsLogFile, '********* Severe exception detected - ' +
    DateTimeToStr(Now) + ' *********');
  Writeln(SevereExceptionsLogFile, VersionInfo);
  Writeln(SevereExceptionsLogFile, 'Exception code: ' + Exc.ClassName);
  Writeln(SevereExceptionsLogFile, 'Exception address: ' +
    TextualDebugInfoForAddress(Cardinal(Addr)));
...
constructor TUnitDebugInfos.Create(const AName: string;
  const NbLines: Cardinal);
begin
  Name := AName;
...
constructor TRoutineDebugInfos.Create(const AName: string;
  const AStartAddress: Cardinal; const ALength: Cardinal);
begin
  Name := AName;
...
function TUnitDebugInfos.LineWhichContainsAddress(const Address
  : Cardinal): string;
var
...
procedure dumpsymbols(NameTbl: PArrayOfPointer; sstptr: PArrayOfByte;
  Size: Integer);
...
begin
  ...
  begin
    ...
    if ((sym = $205) or (sym = $204)) and (PInteger(@sstptr^[40])^ > 0) then
      AddRoutine(PChar(NameTbl^[PInteger(@sstptr^[40])^ - 1]),
        PInteger(@sstptr^[28])^, PInteger(@sstptr^[16])^);
...
procedure dumplines(NameTbl: PArrayOfPointer; sstptr: PArrayOfByte;
  Size: Integer);
// Copyright (C) Tenth Planet Software Intl., Clive Turvey 1998. All rights reserved. - Reused & modified by SG with permission
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        CurrentUnit := TUnitDebugInfos.Create
          (ExtractFileName(PChar(NameTbl^[SRCFILE^._nName - 1])),
          SRCLN^._cPair);
        AddUnit(CurrentUnit);

        for k := 0 to pred(SRCLN^._cPair) do
          CurrentUnit.Addresses[k] :=
            TAddressToLine.Create(Integer(PArrayOfPointer(@SRCLN^._Offset[0])
            ^[k]), Integer(PArrayOfWord(@SRCLN^._Offset[SRCLN^._cPair])^[k]));
      end;
    end;
  end;
end;
...
// Copyright (C) Tenth Planet Software Intl., Clive Turvey 1998. All rights reserved. - Reused & modified by SG with permission
var
  AHeader: packed record Signature: array [0 .. 3] of char;
  AnInteger: Integer;
end;
k:
Integer;
...
begin
  ...
  begin
    SetDispl;
    OutOfMemory := EOutOfMemory.Create
      ('Memcheck is not able to allocate memory, due to system resource lack');
...
begin
  ...
  case Block.KindOfBlock of
    MClass:
      Writeln(F, 'Instance of ',
        Tobject(PChar(Block) + SizeOf(TMemoryBlocHeader)).ClassName);
    MUser:
  ...
  begin
    Write(F, #9'Was field #', fOtherFieldIndex, ' of an instance of ',
      fOwnerClassName);
...
begin
  Result := (fBlock.KindOfBlock = Other.Block.KindOfBlock) and
    (fBlock.AllocatedSize = Other.Block.AllocatedSize);

  if fBlock.KindOfBlock = MClass then
    Result := Result and (Tobject(PChar(fBlock) + SizeOf(TMemoryBlocHeader))
      .ClassName = Tobject(PChar(Other.Block) + SizeOf(TMemoryBlocHeader))
      .ClassName);

  Result := Result and
    (WasFieldOfAnotherObject = Other.WasFieldOfAnotherObject);

  if WasFieldOfAnotherObject then
    Result := Result and (OtherObjectClassName = Other.OtherObjectClassName) and
      (OtherFieldIndex = Other.OtherFieldIndex) and
      (OtherIsDestroyed = Other.OtherIsDestroyed);

  Result := Result and CallStacksEqual(fBlock.CallerAddress,
    Other.Block.CallerAddress)
end;
...
begin
  Assert((I >= 0) and (I < fCount),
    Format('TLeakList.Item: out of bounds (%d)', [I]));
...
procedure GetLeaks(const LeaksList, ChronogicalInfo: TLeakList;
  const MaxNumberOfLeaks: Integer; var StoppedDueToMaxLeak: Boolean);
var
  Block: PMemoryBlocHeader;
  CurrentLeak: TLeak;
  I: Integer;
...
procedure GetBadBlocks(const B: TBlockList;
  const MaxNumberOfBlocks, MaxBlockSize: Integer;
  var StoppedDueToMaxBlock: Boolean);
var
  Block: PMemoryBlocHeader;
begin
  ...
  begin
    if MemoryBlockFreed(Block) and (Block.AllocatedSize > 5) and
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if TLeak(TopLeakRefs[j]).TotalSize < TLeak(TopLeakRefs[MinIdx]).TotalSize
        then
          MinIdx := j;
      end; // for
  ...
  if StoppedDueToMax then
    Writeln(OutputFile,
      'Total leak not accurate due to MaxLeak constant reached, but at least ',
  ...
  Writeln(OutputFile,
    #13#10'*** MEMCHK: Chronological leak information ***'#13#10);
  ...
  begin
    if (TimeStampsCount > 0) and (ChronogicalInfo.Item(I).Block.LastTimeStamp >
      LastDisplayedTimeStamp) then
    begin
      for j := LastDisplayedTimeStamp + 1 to ChronogicalInfo.Item(I)
        .Block.LastTimeStamp do
  ...
  for j := LastDisplayedTimeStamp + 1 to TimeStampsCount - 1 do
    Writeln(OutputFile, '  Time stamp: "', TimeStamps[j], '"');
  Writeln(OutputFile,
    #13#10'*** MEMCHK: End of chronological leak information ***'#13#10);
  ...
  begin
    ...
    if TotalLeak > 0 then
      Writeln(OutputFile,
        #9'The information is not accurate since there are memory leaks'#13#10);
    Writeln(OutputFile, #9'Nb instances'#9'Instance size'#9'ClassName');
    for I := 0 to AllocStatsCount - 1 do
      Writeln(OutputFile, #9, AllocatedInstances[I], #9#9,
        AllocatedObjectsClasses[I].InstanceSize, #9#9,
        AllocatedObjectsClasses[I].ClassName);
  ...
  begin
    ...
    Writeln(OutputFile,
      #13#10'*** MEMCHK: End of memory usage stats ***'#13#10);
  end;
  ...
  begin
    GetBadBlocks(BadBlocks, MaxLeak, DoNotCheckWipedBlocksBiggerThan,
      StoppedDueToMax);

    Writeln(OutputFile,
      #13#10'*** MEMCHK: Blocks written to after destruction ***'#13#10);
    if StoppedDueToMax then
      Writeln(OutputFile,
    ...
    begin
      ...
      Write(OutputFile, CallStackTextualRepresentation(BadBlocks.Item(I)
        .CallerAddress, #9#9#9));
      Writeln(OutputFile, #9#9'Destroyed at: ',
        TextualDebugInfoForAddress(Cardinal(BadBlocks.Item(I)
        .DestructionAdress)));
    end;

    Writeln(OutputFile,
      #13#10'*** MEMCHK: End of blocks written to after destruction ***'#13#10);
  end;
  BadBlocks.Destroy;
  ...
  if ShowLogFileWhenUseful and (LeaksList.count > 0) or
    CollectStatsAboutObjectAllocation or ComputeMemoryUsageStats or KeepMaxMemoryUsage
  then
...
begin
  ...
  begin
    ...
    ZeroMemory(Pointer(Integer(TimeStamps) + TimeStampsCount *
      SizeOf(WideString)), (TimeStampsAllocated - TimeStampsCount) *
      SizeOf(WideString));
    MemChk;
  end;

  TimeStamps[TimeStampsCount] := I + ' (Time stamp: ' +
    IntToStr(TimeStampsCount) + ')';
  TimeStampsCount := TimeStampsCount + 1;
end;

procedure MemChk;
const
  LeakTrackingMemoryManager: TMemoryManager = (GetMem: LeakTrackingGetMem;
    FreeMem: LeakTrackingFreeMem; ReallocMem: LeakTrackingReallocMem;);
  HeapCheckingMemoryManager: TMemoryManager = (GetMem: HeapCheckingGetMem;
    FreeMem: HeapCheckingFreeMem; ReallocMem: HeapCheckingReallocMem;);
begin
  Assert(SizeOf(TMemoryBlocHeader) mod 8 = 0,
    'SizeOf(TMemoryBlocHeader) in MemCheck should be a multiple of 8');
...
begin
  ...
  begin
    Result := Result + LineHeader + 'call stack - ' + IntToStr(I) + ' : ' +
      TextualDebugInfoForAddress(Cardinal(S[I])) + #13#10;
    I := I + 1;
  end;
end;
...
var
  ...
  NTOptHeader: PImageOptionalHeader32;
begin
  ...
  NTHeader := PImageFileHeader(Cardinal(PImageDosHeader(HInstance)._lfanew) +
    HInstance + 4); { SizeOf(IMAGE_NT_SIGNATURE) = 4 }
  NTOptHeader := PImageOptionalHeader32(Cardinal(NTHeader) +
...
{$IFNDEF USE_JEDI_JCL}
  ...
  begin
    ...
    if U <> nil then
      Result := 'Module ' + U.Name + RoutineWhichContainsAddress
        (AddressInDebugInfos) + U.LineWhichContainsAddress(AddressInDebugInfos)
    else
