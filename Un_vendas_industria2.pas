...
procedure TFr_vendas_industria2.AtualizarCamposDevolucao;
var
  Devolucao, Conserto: Boolean;
begin
  Devolucao := Lbnom_fop.Caption = 'DEVOLUÇÃO';
  Conserto := Lbnom_fop.Caption = 'CONSERTO';

  TabNfeRef.TabVisible := (not venda) or Devolucao;

  Label11.Visible := not Devolucao;
  Prcod_cliente.Visible := not Devolucao;
  Lbnom_cliente.Visible := not Devolucao;

  Prcod_fornecedor.Visible := (not venda) or Devolucao;
  LbNom_fornecedor.Visible := (not venda) or Devolucao;
  lbForn.Visible := (not venda) or Devolucao;
  lbcidade_forn.Visible := (not venda) or Devolucao;
  lbCnpj_forn.Visible := (not venda) or Devolucao;

  if Devolucao then
  begin
    lbForn.Caption := 'Fornecedor:';
    lbForn.Left := Label11.Left;
    lbForn.Top := Label11.Top;

    Prcod_fornecedor.Left := Prcod_cliente.Left;
    Prcod_fornecedor.Top := Prcod_cliente.Top;
    Prcod_fornecedor.Width := Prcod_cliente.Width;

    LbNom_fornecedor.Left := Lbnom_cliente.Left;
    LbNom_fornecedor.Top := Lbnom_cliente.Top;
    LbNom_fornecedor.Width := 300;
    lbCnpj_forn.Left := 429;
    lbCnpj_forn.Top := Lbnom_cliente.Top;
    lbcidade_forn.Left := 597;
    lbcidade_forn.Top := Lbnom_cliente.Top;
  end
  else
  begin
    lbForn.Caption := 'Fornecedor (Devolução):';
    lbForn.Left := 9;
    lbForn.Top := 203;

    Prcod_fornecedor.Left := 8;
    Prcod_fornecedor.Top := 216;
    Prcod_fornecedor.Width := 66;

    LbNom_fornecedor.Left := 80;
    LbNom_fornecedor.Top := 224;
    LbNom_fornecedor.Width := 3;
    lbCnpj_forn.Left := 429;
    lbCnpj_forn.Top := 220;
    lbcidade_forn.Left := 597;
    lbcidade_forn.Top := 220;
  end;

  if (mform = 'vendas_devolucoes') or Devolucao or Conserto then
  begin
...
begin
  ...
  if (Lbnom_fop.Caption = 'TROCA') or (Lbnom_fop.Caption = 'PERMUTA') or (Lbnom_fop.Caption = 'CONSERTO') then
  begin
    troca := true;
  end
...
begin
  ...
  if (Lbnom_fop.Caption <> 'DEVOLUÇÃO') and (Lbnom_fop.Caption <> 'CONSERTO') then
  begin
...
begin
  ...
  if (mform = 'vendas_devolucoes') or (Lbnom_fop.Caption = 'DEVOLUÇÃO') or (Lbnom_fop.Caption = 'CONSERTO') then
  begin
    MeTrib_ICMS.Enabled := true;
