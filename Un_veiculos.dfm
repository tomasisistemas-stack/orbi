object fr_veiculo: Tfr_veiculo
  Left = 242
  Top = 146
  BorderIcons = [biSystemMenu]
  Caption = 'Cadastro de veiculos'
  ClientHeight = 504
  ClientWidth = 659
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCloseQuery = FormCloseQuery
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TsPanel
    Left = 0
    Top = 463
    Width = 659
    Height = 41
    Align = alBottom
    TabOrder = 0
    SkinData.SkinSection = 'PANEL'
    object btnov: TsBitBtn
      Left = 5
      Top = 6
      Width = 83
      Height = 31
      Caption = 'Inserir'
      Glyph.Data = {
        
        FFFFFFFFFF00}
      TabOrder = 5
      OnClick = btnovClick
      SkinData.SkinSection = 'BUTTON'
    end
    object btalt: TsBitBtn
      Left = 87
      Top = 6
      Width = 83
      Height = 31
      Caption = 'Alterar'
      Glyph.Data = {
        
        FFFFFFFFFF00}
      TabOrder = 3
      OnClick = btaltClick
      SkinData.SkinSection = 'BUTTON'
    end
    object btgra: TsBitBtn
      Left = 169
      Top = 6
      Width = 83
      Height = 31
      Caption = 'Gravar'
      Enabled = False
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      TabOrder = 4
      OnClick = btgraClick
      SkinData.SkinSection = 'BUTTON'
    end
    object btcan: TsBitBtn
      Left = 251
      Top = 6
      Width = 83
      Height = 31
      Caption = 'Cancelar'
      Enabled = False
      Glyph.Data = {
        
        FFFFFFFFFF00}
      TabOrder = 0
      OnClick = btcanClick
      SkinData.SkinSection = 'BUTTON'
    end
    object btexc: TsBitBtn
      Left = 333
      Top = 6
      Width = 83
      Height = 31
      Caption = 'Excluir'
      Glyph.Data = {
        
        FFFFFFFFFF00}
      TabOrder = 1
      OnClick = btexcClick
      SkinData.SkinSection = 'BUTTON'
    end
    object btpro: TsBitBtn
      Left = 416
      Top = 6
      Width = 77
      Height = 31
      Caption = '&Localizar'
      TabOrder = 2
      OnClick = btproClick
      SkinData.SkinSection = 'BUTTON'
    end
  end
  object pgVeiculos: TsPageControl
    Left = 0
    Top = 0
    Width = 659
    Height = 463
    ActivePage = TabVeiculo
    Align = alClient
    TabOrder = 1
    SkinData.SkinSection = 'PAGECONTROL'
    object TabVeiculo: TsTabSheet
      Caption = 'Dados'
      SkinData.CustomColor = False
      SkinData.CustomFont = False
      object Label1: TsLabel
        Left = 9
        Top = 6
        Width = 33
        Height = 13
        Caption = 'C'#243'digo'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object Label2: TsLabel
        Left = 9
        Top = 45
        Width = 80
        Height = 13
        Caption = 'Nome do veiculo'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object Label3: TsLabel
        Left = 9
        Top = 87
        Width = 27
        Height = 13
        Caption = 'Placa'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object Label4: TsLabel
        Left = 165
        Top = 6
        Width = 86
        Height = 13
        Caption = 'Data de aquisi'#231#227'o'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object sLabel5: TsLabel
        Left = 9
        Top = 297
        Width = 93
        Height = 13
        Caption = 'Kilometragem Inicial'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object sLabel8: TsLabel
        Left = 86
        Top = 87
        Width = 46
        Height = 13
        Caption = 'Renavam'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object sLabel9: TsLabel
        Left = 195
        Top = 87
        Width = 22
        Height = 13
        Caption = 'Tara'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object sLabel10: TsLabel
        Left = 266
        Top = 87
        Width = 38
        Height = 13
        Caption = 'Cap. Kg'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object sLabel11: TsLabel
        Left = 334
        Top = 87
        Width = 34
        Height = 13
        Caption = 'Cap.M'#179
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object sLabel12: TsLabel
        Left = 370
        Top = 6
        Width = 14
        Height = 13
        Caption = 'UF'
        ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
      end
      object Prid: TsEdit
        Left = 9
        Top = 21
        Width = 72
        Height = 21
        Color = clWhite
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        OnEnter = PridEnter
        OnKeyPress = PridKeyPress
        SkinData.SkinSection = 'EDIT'
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
      end
      object Prnom_veiculo: TsEdit
        Left = 9
        Top = 59
        Width = 387
        Height = 21
        CharCase = ecUpperCase
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 50
        ParentFont = False
        TabOrder = 3
        OnKeyPress = Prnom_veiculoKeyPress
        SkinData.SkinSection = 'EDIT'
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
      end
      object Prdt_aquisicao: TsDateEdit
        Left = 165
        Top = 21
        Width = 99
        Height = 21
        AutoSize = False
        Color = clWhite
        EditMask = '!99/99/9999;1; '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 10
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
        Text = '  /  /    '
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
        GlyphMode.Blend = 0
        GlyphMode.Grayed = False
      end
      object PrHodometro_Inicial: TsCurrencyEdit
        Left = 8
        Top = 313
        Width = 80
        Height = 21
        AutoSize = False
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 11
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
        GlyphMode.Blend = 0
        GlyphMode.Grayed = False
      end
      object gbProximaRevisao: TsGroupBox
        Left = 179
        Top = 297
        Width = 217
        Height = 65
        Caption = 'Pr'#243'xima Revis'#227'o'
        TabOrder = 12
        SkinData.SkinSection = 'GROUPBOX'
        object sLabel6: TsLabel
          Left = 9
          Top = 20
          Width = 23
          Height = 13
          Caption = 'Data'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
        end
        object sLabel7: TsLabel
          Left = 123
          Top = 20
          Width = 66
          Height = 13
          Caption = 'Kilometragem '
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
        end
        object PrPROXIMA_REVISAO_DATA: TsDateEdit
          Left = 8
          Top = 34
          Width = 99
          Height = 21
          AutoSize = False
          Color = clWhite
          EditMask = '!99/99/9999;1; '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          Text = '  /  /    '
          BoundLabel.Indent = 0
          BoundLabel.Font.Charset = DEFAULT_CHARSET
          BoundLabel.Font.Color = clWindowText
          BoundLabel.Font.Height = -11
          BoundLabel.Font.Name = 'MS Sans Serif'
          BoundLabel.Font.Style = []
          BoundLabel.Layout = sclLeft
          BoundLabel.MaxWidth = 0
          BoundLabel.UseSkinColor = True
          SkinData.SkinSection = 'EDIT'
          GlyphMode.Blend = 0
          GlyphMode.Grayed = False
        end
        object PrPROXIMA_REVISAO_HODOMETRO: TsCurrencyEdit
          Left = 122
          Top = 34
          Width = 80
          Height = 21
          AutoSize = False
          Color = clWhite
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          BoundLabel.Indent = 0
          BoundLabel.Font.Charset = DEFAULT_CHARSET
          BoundLabel.Font.Color = clWindowText
          BoundLabel.Font.Height = -11
          BoundLabel.Font.Name = 'MS Sans Serif'
          BoundLabel.Font.Style = []
          BoundLabel.Layout = sclLeft
          BoundLabel.MaxWidth = 0
          BoundLabel.UseSkinColor = True
          SkinData.SkinSection = 'EDIT'
          GlyphMode.Blend = 0
          GlyphMode.Grayed = False
          DecimalPlaces = 0
          DisplayFormat = '### ### ##0.00;-### ### ##0;0'
        end
      end
      object PrRenavam: TsMaskEdit
        Left = 86
        Top = 101
        Width = 98
        Height = 21
        CharCase = ecUpperCase
        Color = clWhite
        EditMask = '99999999999999;0; '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 14
        ParentFont = False
        TabOrder = 5
        Text = ''
        CheckOnExit = True
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
      end
      object PrTara: TsCurrencyEdit
        Left = 195
        Top = 101
        Width = 66
        Height = 21
        AutoSize = False
        TabOrder = 6
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'Tahoma'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
        GlyphMode.Blend = 0
        GlyphMode.Grayed = False
      end
      object PrCapKg: TsCurrencyEdit
        Left = 266
        Top = 101
        Width = 62
        Height = 21
        AutoSize = False
        TabOrder = 7
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'Tahoma'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
        GlyphMode.Blend = 0
        GlyphMode.Grayed = False
      end
      object PrCapm3: TsCurrencyEdit
        Left = 334
        Top = 101
        Width = 62
        Height = 21
        AutoSize = False
        TabOrder = 8
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'Tahoma'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
        SkinData.SkinSection = 'EDIT'
        GlyphMode.Blend = 0
        GlyphMode.Grayed = False
      end
      object PrTipoRodado: TsRadioGroup
        Left = 9
        Top = 128
        Width = 387
        Height = 81
        Caption = 'Tipo de Rodado'
        TabOrder = 9
        SkinData.SkinSection = 'GROUPBOX'
        Columns = 3
        ItemIndex = 0
        Items.Strings = (
          'N'#227'o Aplicavel,'
          'Truck '
          'Toco'
          'Cavalo Mec'#226'nico '
          'VAN'
          'Utilit'#225'rio'
          'Outros')
      end
      object PrTipoCarroceria: TsRadioGroup
        Left = 9
        Top = 210
        Width = 387
        Height = 81
        Caption = 'Tipo de Carroceria'
        TabOrder = 10
        SkinData.SkinSection = 'GROUPBOX'
        Columns = 3
        ItemIndex = 0
        Items.Strings = (
          'N'#227'o Aplic'#225'vel '
          'Aberta'
          'Fechada'
          'Graneleira'
          'Porta Container, '
          'Sider')
      end
      object PrUF: TsEdit
        Left = 370
        Top = 21
        Width = 26
        Height = 21
        CharCase = ecUpperCase
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 50
        ParentFont = False
        TabOrder = 2
        OnKeyPress = Prnom_veiculoKeyPress
        SkinData.SkinSection = 'EDIT'
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
      end
      object Prplaca: TsEdit
        Left = 9
        Top = 101
        Width = 66
        Height = 21
        CharCase = ecUpperCase
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 9
        ParentFont = False
        TabOrder = 4
        SkinData.SkinSection = 'EDIT'
        BoundLabel.Indent = 0
        BoundLabel.Font.Charset = DEFAULT_CHARSET
        BoundLabel.Font.Color = clWindowText
        BoundLabel.Font.Height = -11
        BoundLabel.Font.Name = 'MS Sans Serif'
        BoundLabel.Font.Style = []
        BoundLabel.Layout = sclLeft
        BoundLabel.MaxWidth = 0
        BoundLabel.UseSkinColor = True
      end
    end
    object TabDespesas: TsTabSheet
      Caption = 'Despesas'
      SkinData.CustomColor = False
      SkinData.CustomFont = False
      object sPanel1: TsPanel
        Left = 0
        Top = 0
        Width = 651
        Height = 32
        Align = alTop
        BevelInner = bvLowered
        BevelOuter = bvLowered
        TabOrder = 0
        SkinData.SkinSection = 'PANEL_LOW'
        object Lbl_Colaborador: TsLabel
          Left = 17
          Top = 10
          Width = 57
          Height = 13
          Caption = 'Fornecedor:'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
        end
        object LbFornecedor: TsLabel
          Left = 158
          Top = 11
          Width = 9
          Height = 13
          Caption = '...'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
        end
        object sLabel2: TsLabel
          Left = 401
          Top = 10
          Width = 17
          Height = 13
          Caption = 'De:'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
        end
        object sLabel3: TsLabel
          Left = 526
          Top = 10
          Width = 18
          Height = 13
          Caption = 'at'#233':'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
        end
        object sLabel4: TsLabel
          Left = 265
          Top = 10
          Width = 44
          Height = 13
          Caption = 'N'#186' Nota: '
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
        end
        object ftcod_fornecedor: TsComboEdit
          Left = 77
          Top = 6
          Width = 77
          Height = 21
          AutoSize = False
          Color = clWhite
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          Text = ''
          OnExit = ftcod_fornecedorExit
          CheckOnExit = True
          BoundLabel.Indent = 0
          BoundLabel.Font.Charset = DEFAULT_CHARSET
          BoundLabel.Font.Color = clWindowText
          BoundLabel.Font.Height = -11
          BoundLabel.Font.Name = 'MS Sans Serif'
          BoundLabel.Font.Style = []
          BoundLabel.Layout = sclLeft
          BoundLabel.MaxWidth = 0
          BoundLabel.UseSkinColor = True
          SkinData.SkinSection = 'EDIT'
          OnButtonClick = ftcod_fornecedorButtonClick
          GlyphMode.Blend = 0
          GlyphMode.Grayed = False
        end
        object ftMovDe: TsDateEdit
          Left = 421
          Top = 6
          Width = 99
          Height = 21
          AutoSize = False
          Color = clWhite
          EditMask = '!99/99/9999;1; '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          TabOrder = 1
          Text = '  /  /    '
          BoundLabel.Indent = 0
          BoundLabel.Font.Charset = DEFAULT_CHARSET
          BoundLabel.Font.Color = clWindowText
          BoundLabel.Font.Height = -11
          BoundLabel.Font.Name = 'MS Sans Serif'
          BoundLabel.Font.Style = []
          BoundLabel.Layout = sclLeft
          BoundLabel.MaxWidth = 0
          BoundLabel.UseSkinColor = True
          SkinData.SkinSection = 'EDIT'
          GlyphMode.Blend = 0
          GlyphMode.Grayed = False
          DialogTitle = 'Selecione uma Data'
        end
        object ftMovAte: TsDateEdit
          Left = 547
          Top = 6
          Width = 99
          Height = 21
          AutoSize = False
          Color = clWhite
          EditMask = '!99/99/9999;1; '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          TabOrder = 2
          Text = '  /  /    '
          BoundLabel.Indent = 0
          BoundLabel.Font.Charset = DEFAULT_CHARSET
          BoundLabel.Font.Color = clWindowText
          BoundLabel.Font.Height = -11
          BoundLabel.Font.Name = 'MS Sans Serif'
          BoundLabel.Font.Style = []
          BoundLabel.Layout = sclLeft
          BoundLabel.MaxWidth = 0
          BoundLabel.UseSkinColor = True
          SkinData.SkinSection = 'EDIT'
          GlyphMode.Blend = 0
          GlyphMode.Grayed = False
          DialogTitle = 'Selecione uma Data'
        end
        object ftNota: TsCurrencyEdit
          Left = 311
          Top = 6
          Width = 80
          Height = 21
          AutoSize = False
          Color = clWhite
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 3
          BoundLabel.Indent = 0
          BoundLabel.Font.Charset = DEFAULT_CHARSET
          BoundLabel.Font.Color = clWindowText
          BoundLabel.Font.Height = -11
          BoundLabel.Font.Name = 'MS Sans Serif'
          BoundLabel.Font.Style = []
          BoundLabel.Layout = sclLeft
          BoundLabel.MaxWidth = 0
          BoundLabel.UseSkinColor = True
          SkinData.SkinSection = 'EDIT'
          GlyphMode.Blend = 0
          GlyphMode.Grayed = False
        end
      end
      object sDBGrid1: TDBGrid
        Left = 0
        Top = 32
        Width = 651
        Height = 345
        Align = alClient
        Color = clWhite
        DataSource = ds_despesas
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        TabOrder = 1
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'nr_documento'
            Title.Alignment = taCenter
            Title.Caption = 'Nota'
            Width = 80
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'dta_documento'
            Title.Alignment = taCenter
            Title.Caption = 'Data'
            Width = 75
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'fornecedor'
            Title.Alignment = taCenter
            Title.Caption = 'Fornecedor'
            Width = 364
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'vl_total'
            Title.Alignment = taCenter
            Title.Caption = 'Total'
            Width = 90
            Visible = True
          end>
      end
      object Panel3: TsPanel
        Left = 0
        Top = 401
        Width = 651
        Height = 34
        Align = alBottom
        BevelInner = bvLowered
        TabOrder = 2
        SkinData.SkinSection = 'PANEL'
        object BtNovEm: TsBitBtn
          Left = 201
          Top = 5
          Width = 75
          Height = 25
          Caption = '&Inserir'
          TabOrder = 0
          OnClick = BtNovEmClick
          SkinData.SkinSection = 'BUTTON'
        end
        object BtAltEm: TsBitBtn
          Left = 288
          Top = 5
          Width = 75
          Height = 25
          Caption = '&Alterar'
          TabOrder = 1
          OnClick = BtAltEmClick
          SkinData.SkinSection = 'BUTTON'
        end
        object BtEmConsulta: TsBitBtn
          Left = 374
          Top = 5
          Width = 75
          Height = 25
          Caption = '&Consulta'
          TabOrder = 2
          OnClick = BtEmConsultaClick
          SkinData.SkinSection = 'BUTTON'
        end
      end
      object pnSaldo: TsPanel
        Left = 0
        Top = 377
        Width = 651
        Height = 24
        Align = alBottom
        BevelOuter = bvLowered
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -21
        Font.Name = 'Calibri'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 3
        SkinData.SkinSection = 'PANEL_LOW'
        object sBevel1: TsBevel
          Left = 504
          Top = 4
          Width = 118
          Height = 17
        end
        object sDBTextFX1: TsDBTextFX
          Left = 506
          Top = 3
          Width = 116
          Height = 18
          Alignment = taRightJustify
          Caption = 'sDBTextFX1'
          ParentFont = False
          ShowAccelChar = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Calibri'
          Font.Style = [fsBold]
          Angle = 0
          Shadow.OffsetKeeper.LeftTop = -3
          Shadow.OffsetKeeper.RightBottom = 5
          DataField = 'TOTAL'
          DataSource = ds_total_despesas
        end
        object sLabel1: TsLabel
          Left = 470
          Top = 4
          Width = 30
          Height = 15
          Caption = 'Total:'
          ParentFont = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = [fsBold]
        end
      end
    end
    object tabConsumo: TsTabSheet
      Caption = 'Consumo'
      SkinData.CustomColor = False
      SkinData.CustomFont = False
      object sDBGrid2: TDBGrid
        Left = 0
        Top = 0
        Width = 651
        Height = 435
        Align = alClient
        Color = clWhite
        DataSource = ds_consumo
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'nr_documento'
            Title.Alignment = taCenter
            Title.Caption = 'Nota'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'dta_documento'
            Title.Alignment = taCenter
            Title.Caption = 'Data'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'fornecedor'
            Title.Alignment = taCenter
            Title.Caption = 'Fornecedor'
            Width = 120
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'produto'
            Title.Alignment = taCenter
            Title.Caption = 'Combust'#237'vel'
            Width = 120
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'hodometro_anterior'
            Title.Alignment = taCenter
            Title.Caption = 'Km Ant'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'hodometro'
            Title.Alignment = taCenter
            Title.Caption = 'Km'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'qtd_entrada'
            Title.Caption = 'Quantidade'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'media_consumo'
            Title.Alignment = taCenter
            Title.Caption = 'M'#233'dia'
            Width = 60
            Visible = True
          end>
      end
    end
  end
  object q_veiculos: TFDQuery
    Connection = dao.CN
    Left = 576
    Top = 184
  end
  object q_despesas: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      'SELECT DISTINCT'
      '  e2.ID_VEICULO,'
      '  e1.NR_DOCUMENTO,'
      '  e1.DTA_DOCUMENTO, '
      '  f.COD_FORNECEDOR,'
      '  f.COD_FORNECEDOR||'#39'-'#39'||f.RAZAO_SOCIAL as FORNECEDOR,'
      '  e1.VL_TOTAL'
      'FROM ENTRADA1 e1'
      'INNER JOIN FORNECEDOR f on f.COD_FORNECEDOR = e1.COD_FORNECEDOR'
      
        'INNER JOIN ENTRADA2 e2 on e2.NR_DOCUMENTO = e1.NR_DOCUMENTO and ' +
        'e2.COD_FORNECEDOR = e1.COD_FORNECEDOR'
      'INNER JOIN VEICULOS v on v.ID = e2.ID_VEICULO'
      ''
      'where e2.ID_VEICULO =:veiculo')
    Left = 528
    Top = 184
    ParamData = <
      item
        Name = 'veiculo'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object q_despesasid_veiculo: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'id_veiculo'
      ProviderFlags = []
    end
    object q_despesasnr_documento: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'nr_documento'
      ProviderFlags = []
    end
    object q_despesasdta_documento: TDateField
      AutoGenerateValue = arDefault
      FieldName = 'dta_documento'
      ProviderFlags = []
    end
    object q_despesascod_fornecedor: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'cod_fornecedor'
      ProviderFlags = []
    end
    object q_despesasfornecedor: TMemoField
      AutoGenerateValue = arDefault
      FieldName = 'fornecedor'
      ProviderFlags = []
      BlobType = ftMemo
    end
    object q_despesasvl_total: TBCDField
      AutoGenerateValue = arDefault
      FieldName = 'vl_total'
      ProviderFlags = []
      Precision = 15
      Size = 2
    end
  end
  object ds_despesas: TDataSource
    DataSet = q_despesas
    Left = 476
    Top = 188
  end
  object q_total_despesas: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      'SELECT distinct'
      '  sum(e1.VL_TOTAL) as total'
      'FROM ENTRADA1 e1'
      'where '
      
        '  cast(e1.NR_DOCUMENTO as varchar(20))||cast(e1.COD_FORNECEDOR a' +
        's varchar(20))'
      '  in'
      '( '
      ''
      
        'select cast(e2.NR_DOCUMENTO as varchar(20))||cast(e2.COD_FORNECE' +
        'DOR as varchar(20))'
      'from ENTRADA2 e2 '
      'INNER JOIN VEICULOS v on v.ID = e2.ID_VEICULO'
      'where e2.ID_VEICULO =:veiculo'
      ')')
    Left = 528
    Top = 240
    ParamData = <
      item
        Name = 'VEICULO'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object q_total_despesastotal: TFMTBCDField
      AutoGenerateValue = arDefault
      FieldName = 'total'
      Origin = 'total'
      ReadOnly = True
      Precision = 64
      Size = 0
    end
  end
  object ds_total_despesas: TDataSource
    DataSet = q_total_despesas
    Left = 476
    Top = 244
  end
  object q_consumo: TFDQuery
    Connection = dao.CN
    SQL.Strings = (
      
        'SELECT distinct E2.ID, E2.NR_DOCUMENTO, E1.DTA_DOCUMENTO, F.COD_' +
        'FORNECEDOR||'#39'-'#39'||F.RAZAO_SOCIAL AS FORNECEDOR, P.COD_PRODUTO||'#39'-' +
        #39'||P.NOM_PRODUTO AS PRODUTO,  '
      ''
      'coalesce('
      '  ('
      '    select '
      '      max(e3.HODOMETRO) '
      '    from '
      '      ENTRADA2 e3'
      '    INNER JOIN PRODUTO P1 ON P1.COD_PRODUTO = E3.COD_PRODUTO'
      
        '    INNER JOIN PLANO_CONTAS PC2 ON PC2.ID = P1.ID_PLANO_CONTAS  ' +
        '     '
      '    where '
      '      PC2.ID = PC.ID AND'
      '      e3.ID_VEICULO = e2.ID_VEICULO and '
      '      e3.HODOMETRO < e2.HODOMETRO'
      '  ), V.HODOMETRO_INICIAL) as hodometro_anterior, '
      'E2.HODOMETRO, E2.QTD_ENTRADA, E2.CUSTO_FINAL, E2.SUB_TOTAL,'
      'e2.QTD_ENTRADA '
      '/ '
      '('
      ' e2.HODOMETRO - '
      ' '
      ' coalesce('
      '  ('
      '    select '
      '      max(e3.HODOMETRO) '
      '    from '
      '      ENTRADA2 e3'
      '    INNER JOIN PRODUTO P1 ON P1.COD_PRODUTO = E3.COD_PRODUTO'
      
        '    INNER JOIN PLANO_CONTAS PC2 ON PC2.ID = P1.ID_PLANO_CONTAS  ' +
        '     '
      '    where '
      '      PC2.ID = PC.ID AND'
      '      e3.ID_VEICULO = e2.ID_VEICULO and '
      '      e3.HODOMETRO < e2.HODOMETRO'
      '  ), V.HODOMETRO_INICIAL)'
      ''
      ') as media_consumo'
      'FROM ENTRADA2 E2'
      'INNER JOIN VEICULOS V ON V.ID = E2.ID_VEICULO'
      
        'INNER JOIN ENTRADA1 E1 ON E1.COD_FORNECEDOR = E2.COD_FORNECEDOR ' +
        'AND E1.NR_DOCUMENTO = E2.NR_DOCUMENTO'
      'INNER JOIN FORNECEDOR F ON F.COD_FORNECEDOR = E2.COD_FORNECEDOR'
      'INNER JOIN PRODUTO P ON P.COD_PRODUTO = E2.COD_PRODUTO'
      'INNER JOIN PLANO_CONTAS PC ON PC.ID = P.ID_PLANO_CONTAS'
      'where'
      '  E2.ID_VEICULO = :veiculo and'
      '  PC.DESCRICAO LIKE '#39'%COMB'#218'ST'#205'VEIS%'#39' and'
      '  e1.DTA_DOCUMENTO between current_date - 30 and current_date'
      'order by dta_documento desc, HODOMETRO DESC')
    Left = 528
    Top = 288
    ParamData = <
      item
        Name = 'VEICULO'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object q_consumoid: TIntegerField
      FieldName = 'id'
      Origin = 'id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object q_consumonr_documento: TIntegerField
      FieldName = 'nr_documento'
      Origin = 'nr_documento'
    end
    object q_consumodta_documento: TDateField
      AutoGenerateValue = arDefault
      FieldName = 'dta_documento'
      Origin = 'dta_documento'
    end
    object q_consumofornecedor: TMemoField
      AutoGenerateValue = arDefault
      FieldName = 'fornecedor'
      Origin = 'fornecedor'
      ReadOnly = True
      BlobType = ftMemo
    end
    object q_consumoproduto: TMemoField
      AutoGenerateValue = arDefault
      FieldName = 'produto'
      Origin = 'produto'
      ReadOnly = True
      BlobType = ftMemo
    end
    object q_consumohodometro_anterior: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'hodometro_anterior'
      Origin = 'hodometro_anterior'
      ReadOnly = True
    end
    object q_consumohodometro: TIntegerField
      FieldName = 'hodometro'
      Origin = 'hodometro'
    end
    object q_consumoqtd_entrada: TBCDField
      FieldName = 'qtd_entrada'
      Origin = 'qtd_entrada'
      Precision = 15
      Size = 2
    end
    object q_consumocusto_final: TBCDField
      FieldName = 'custo_final'
      Origin = 'custo_final'
      Precision = 15
      Size = 2
    end
    object q_consumosub_total: TBCDField
      FieldName = 'sub_total'
      Origin = 'sub_total'
      Precision = 15
      Size = 2
    end
    object q_consumomedia_consumo: TFMTBCDField
      AutoGenerateValue = arDefault
      FieldName = 'media_consumo'
      Origin = 'media_consumo'
      ReadOnly = True
      Precision = 64
      Size = 0
    end
  end
  object ds_consumo: TDataSource
    DataSet = q_consumo
    Left = 476
    Top = 292
  end
end
