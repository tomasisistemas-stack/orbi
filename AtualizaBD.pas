...
{    script.Add('CREATE VIEW ORDEM_CATALOGO( '+
  ...
  if versao_banco < '2.00.85' then
  begin
    script.Add('CREATE TABLE IF NOT EXISTS FOP_PRAZO (COD_FOP INTEGER NOT NULL, ID_PRAZO INTEGER NOT NULL, PADRAO CHAR(1) DEFAULT ''N'', PRIMARY KEY (COD_FOP, ID_PRAZO));');
    script.Add('CREATE INDEX IF NOT EXISTS IDX_FOP_PRAZO_PRAZO ON FOP_PRAZO (ID_PRAZO);');
    script.Add('INSERT INTO FOP_PRAZO (COD_FOP, ID_PRAZO, PADRAO) SELECT COD_FOP, PRAZO_PADRAO, ''S'' FROM FOP WHERE PRAZO_PADRAO IS NOT NULL AND NOT EXISTS (SELECT 1 FROM FOP_PRAZO FP WHERE FP.COD_FOP = FOP.COD_FOP AND FP.ID_PRAZO = FOP.PRAZO_PADRAO);');
